    ...
    return (
        <>
            ...
            <section className="title w-full px-5 lg:px-10 mb-20">
                ...
                <p className="lg:w-3/4">We aim to respond to all enquiries as soon as possible, but there might be a slight delay if the weather is good.</p>
            </section>
            <section className="w-full px-5 lg:px-10 mb-5 grid lg:grid-cols-3 lg:gap-5">
                <section className="lg:col-span-2 mb-5">
                    {showSuccess && <p className="text-green bg-green bg-opacity-25 px-3.5 py-2 mb-5">Thanks for getting in touch.</p>}
                    {showError && <p className="text-red bg-red bg-opacity-25 px-3.5 py-2 mb-5">Please correct the errors below.</p>}
                    <form onSubmit={onSubmit}>
                        ...
                        <fieldset className="mb-5">
                            ...
                            <select name="query" id="query" aria-describedby="query" className={`w-1/2 ${showError && errors.query ? 'border-red bg-red bg-opacity-25 text-red' : ''}`} onChange={e => setQuery(e.target.value)} value={query} required>
                                {options.map(item => <option key={item} value={item}>{item}</option>)}
                            </select>
                        </fieldset>
                        <fieldset className="mb-5">
                            <label htmlFor="info" className="block mb-2">Message</label>
            ...
            <section className="w-full px-5 lg:px-10 mb-20 grid lg:grid-cols-3 lg:gap-5">
                ...
                <section>
                    <h3 className="text-xl font-ropa">Go-Ride coaching meet point</h3>
