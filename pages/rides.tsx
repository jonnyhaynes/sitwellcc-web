            ...
            <section className="w-full px-5 lg:px-10 mb-20 grid lg:grid-cols-2 lg:gap-5">
                <section>
                    ...
                    { events?.data?.sort((a: any, b: any) => a.start.unix - b.start.unix).map((event: any, index: any) => {
                        ...
                        switch (true) {
                            case event.name.includes('RED'):
                            case event.name.includes('Bank Holiday Blowout'):
                            case event.name.includes('Club Competitions'):
                                color = 'red';
                                break;
                            case event.name.includes('AMBER'):
                            case event.name.includes('Wing it Wednesday™ - Road'):
                                color = 'amber';
                                break;
                            ...
                            case event.name.includes('Go-Ride'):
                                color = 'blue';
                                break;
                            default:
                                color = 'black';
                        ...
                        return (
                            <article className={`mb-5 border-t-8 border-${color} p-5 bg-${color} bg-opacity-25 flex flex-row`} key={index}>
                                <picture className="block w-28 h-28 mr-5 overflow-hidden flex-shrink-0">
                                    <Image src={event.images.thumbnail} className="object-cover w-full h-full" alt="An event photograph from Ticket Tailor" width="108" height="108" />
                                </picture>
...
export async function getServerSideProps() {
    ...
    const res = await fetch(`${baseURL}events?start_at.gte=${start}&end_at.lte=${end}&limit=${limit}`, {
        ...
        headers: {
            ...
            'Cache-Control': 'public, s-maxage=60, stale-while-revalidate=599',
        },
    });
